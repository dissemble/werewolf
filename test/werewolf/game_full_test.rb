...
module Werewolf
  ...
  class GameFullTest < Minitest::Test
    ...
    def test_other_roles
      ...
      # start 5 player game
      game.stubs(:define_roles).returns ['seer', 'wolf', 'sasquatch', 'apprentice', 'villager', 'tanner']
      game.start

      seer = game.players.values.find {|p| 'seer' == p.role}
      wolf = game.players.values.find {|p| 'wolf' == p.role}
      sasquatch = game.players.values.find {|p| 'sasquatch' == p.role}
      apprentice = game.players.values.find {|p| 'apprentice' == p.role}
      villager = game.players.values.find {|p| 'villager' == p.role}
      tanner = game.players.values.find {|p| 'tanner' == p.role}
      ...
      game.vote(voter_name: wolf.name, candidate_name: seer.name)
      # all others abstain
      ...
      # Day 2, tie vote
      game.vote(voter_name: sasquatch.name, candidate_name: wolf.name)
      game.vote(voter_name: wolf.name, candidate_name: sasquatch.name)
      # all others abstain
      assert_equal 2, game.remaining_votes.size

      # Dusk
      game.advance_time
      assert 'wolf', sasquatch.role
