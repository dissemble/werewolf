...
module Werewolf
  class EventLoopTest < Minitest::Test
    ...
    def test_next_calls_advance_time_when_round_NOT_expired_but_voting_finished
      game = Game.new
      event_loop = EventLoop.new(game)
    ...
    def test_next_calls_tick_if_no_other_conditions_met
      game = Game.new
      event_loop = EventLoop.new(game)
    ...
    def test_notification_on_warning_tick
      game = Game.new
      event_loop = EventLoop.new(game)
    ...
    def test_no_notification_on_warning_tick
      game = Game.new
      event_loop = EventLoop.new(game)
